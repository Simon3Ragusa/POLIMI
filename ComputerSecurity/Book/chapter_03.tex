...
Questions often ask you to analyze the security of custom authentication schemes (e.g., biometrics, hardware protocols). You should be ready to highlight weaknesses—such as susceptibility to snooping, guessing, or cloning—and suggest appropriate improvements or multi-factor combinations.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
For exams, be prepared to analyze password and PIN entropy and brute-forcing probabilities. You may be asked to calculate the success probability of a brute-force attack given a specific attacker energy budget or hash truncation constraint.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
A typical exam scenario requires analyzing custom password recovery mechanisms. Check if secrets are transmitted in plaintext or poorly stored. Highlight these vulnerabilities and propose secure storage methods (like salted hashes) and robust exchange protocols (like challenge-response) as countermeasures.
\end{tcolorbox}
