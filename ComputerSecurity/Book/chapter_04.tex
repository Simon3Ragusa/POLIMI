...
Questions about this topic are usually based on identifying assets at risk, associated threats, and threat agents for a given scenario (e.g., floor cleaning robots, tracking devices, infrastructure). You must analyze vulnerabilities, explain the exploit mechanics, and propose effective countermeasures and mitigations to reduce risk. Additionally, you may be asked to correct a Risk Analysis Table by evaluating Likelihood/Impact and justifying your changes, as well as to suggest potential attack surfaces for the described system.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
Exam questions often ask you to analyze the security of custom authentication schemes (e.g., password recovery mechanisms, biometric systems, or hardware authentication protocols). You should be prepared to critically highlight weaknesses such as susceptibility to snooping, guessing, or interception attacks. To score full points, clearly outline the vulnerability and propose specific, actionable improvements to secure the authentication process.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
When analyzing vulnerabilities and proposing mitigations for a given scenario, always consider how inherent flaws in access control models—like DAC's vulnerability to Trojan Horses and data flow leakage—can be exploited by an attacker. You will be expected to thoroughly explain the exploit mechanics and propose architectural countermeasures (e.g., migrating to MAC or adding strict isolation) to mitigate the threat.
\end{tcolorbox}
...
This chapter explores how systems make and scale binary decisions regarding resource access using policies. It details the \textbf{Reference Monitor} as the core enforcer, \textbf{Discretionary Access Control (DAC)} where owners assign privileges (which is vulnerable to Trojan Horses), and \textbf{Mandatory Access Control (MAC)} using lattice-based models like Bell-LaPadula to enforce system-wide secrecy.
\end{tcolorbox}
...
\begin{table}[htpb]
    ...
    \begin{tabular}{|l|p{5.5cm}|p{5.5cm}|}
        \hline
        \textbf{Aspect} & \textbf{DAC (Discretionary)} & \textbf{MAC (Mandatory)} \\
        \hline
        \textbf{Authority} & Resource owner assigns privileges & System administrator sets strict policies \\
        \textbf{Implementations} & ACLs, Capability Lists, UNIX Triads & Lattice-Based Access Control (LBAC) \\
        \textbf{Data Flow} & Fails to restrict data flow in memory & Enforces monotonic upward data flow \\
        \textbf{Trojan Horse} & Highly vulnerable & Mitigated via the $\star$-property (No Write Down) \\
        \hline
    \end{tabular}
    \caption{Comparison of DAC and MAC}
    \label{tab:dac_vs_mac}
\end{table}
