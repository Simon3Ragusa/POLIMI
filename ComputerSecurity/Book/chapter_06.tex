...
Questions on this topic often require you to draw the process memory stack layout right before and after the execution of a vulnerable line or exploit, showing exactly where variables, \texttt{ESP}, \texttt{EBP}, and the saved \texttt{EIP} are located.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
You may be asked to identify buffer overflow vulnerabilities directly from provided C source code. Expect to pinpoint the vulnerable lines, explain if they are exploitable based on the context, and propose secure alternatives to fix the code.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
A common exam exercise involves writing exploits: detailing the assumptions, the steps taken, and constructing the exact payload to exploit a stack-based buffer overflow, often to execute shellcode or modify specific program variables.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
Be prepared to analyze mitigations and discuss how they can be bypassed. Questions often ask about the effects of NX, ASLR, or Stack Canaries, and whether alternative techniques (e.g., ret2libc, format string exploits) can be used to circumvent these defenses.
\end{tcolorbox}
