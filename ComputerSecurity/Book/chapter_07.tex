...
A common exam task is to identify vulnerabilities in C source code, specifically Buffer Overflows and Format Strings. Look for functions like \texttt{printf(buf)} or \texttt{snprintf(..., buf)}, where \texttt{buf} is user-controlled. You will be asked to identify the exact vulnerable lines, explain whether and why they are exploitable (e.g., if the vulnerability allows arbitrary reads or writes), and propose a secure fix (e.g., rewriting it as \texttt{printf("\%s", buf)}).
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
Be prepared to write full exploit payloads for Format String bugs. You may need to describe each step, clearly state your assumptions, and construct the precise payload (calculating exact padding for \texttt{\%hn} writes) to execute shellcode or modify variables. Furthermore, exams frequently require drawing the memory stack layout: you must accurately show the process memory right before the execution of the vulnerable line and right after the format string exploit execution.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
You may be asked to analyze the bypass of memory mitigations like NX, ASLR, or Stack Canaries. For instance, using a Format String vulnerability instead of a Buffer Overflow is a classic bypass for Stack Canaries. A standard technique discussed in exams is using a Format String to leak memory (such as a stack canary or a libc base address) before executing a secondary exploit like a Buffer Overflow, or leveraging \texttt{.text} to bypass NX.
\end{tcolorbox}
