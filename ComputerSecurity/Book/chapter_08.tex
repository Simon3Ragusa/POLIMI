...
For XSS, be prepared to identify vulnerabilities in source code, specify the vulnerable lines and variables, and explain how to fix or sanitize them. You must also know how to write the final payload/code to reproduce specific attacks such as Reflected, Stored, or DOM-based XSS.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
SQLi exam questions frequently require identifying vulnerabilities in source code, pointing out the vulnerable variables, and explaining fixes (e.g., Prepared Statements). Furthermore, expect to write payloads to reproduce specific attacks, particularly Blind and UNION-based SQL injections.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
Be prepared to write payloads to reproduce Path Traversal attacks (e.g., manipulating parameters to access files like \texttt{../../../../etc/passwd}).
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
CSRF questions often ask to identify the vulnerability in source code and provide the final payload/code to reproduce the attack (e.g., crafting an HTML form that auto-submits a forged request to the vulnerable endpoint).
\end{tcolorbox}
