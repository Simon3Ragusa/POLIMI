...
For any of these attack categories, exam questions often ask you to analyze network packet captures or logs. Your main goal is to determine the IP and MAC addresses of the attacker and the victim, identify the specific type of attack taking place (e.g., ARP Spoofing, DNS Cache Poisoning, ICMP Redirect, CAM Flooding, TCP session hijacking), and articulate the attacker's main goal or the next steps in their attack chain based on the traffic traces.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
Exam questions often ask you to propose network mitigations or topology changes. Be prepared to suggest and explain mitigations like SYN-cookies to prevent SYN Flood attacks. You should also be able to evaluate the effectiveness of these mitigations (e.g., how SYN-cookies avoid state allocation on the server side) and understand how adding firewalls, DMZs, or VPNs changes the network's resilience against such attacks.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
When analyzing network packet captures that involve an overwhelming number of frames with random source MAC addresses, you should identify it as CAM flooding or MAC flooding. You will need to describe its steps, primarily how it forces a switch into ``fail-open'' mode (acting like a hub) to enable sniffing. Additionally, you will be asked to propose network mitigations such as implementing Port Security to limit the number of MAC addresses per port, and explain how this mitigation prevents the table from filling up.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
ARP Spoofing/Poisoning is a frequent topic in network packet capture analysis questions. You must be able to identify it from a trace (typically spotting gratuitous ARP replies or multiple IPs associated with one MAC) and describe its steps in detail. Be ready to determine the attacker's IP and MAC, the victim's IP, and explain the attacker's ultimate goal, which is often to establish a Man-in-the-Middle (MITM) position to intercept or modify traffic.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
Exam questions may present packet logs and ask you to identify TCP session hijacking. You will need to describe the steps the attacker takes in detail, such as sniffing the connection to obtain current sequence and acknowledgment numbers, disrupting the original connection (e.g., via a targeted DoS against the client), and injecting a spoofed packet to the server with the correct next sequence number to hijack the session.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
DNS Cache Poisoning is often featured in packet analysis questions. Be ready to identify the attack from logs (e.g., seeing a flood of DNS responses trying to guess the Transaction ID) and explain its mechanism. This includes how the attacker spoofs the Transaction ID and UDP source port, floods the local server with forged responses before the legitimate authoritative server replies, and ultimately causes the local server to cache a malicious IP record.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
Questions involving ICMP Redirect often require you to identify the attack from network logs and explain the steps taken to trick a host into routing traffic maliciously. You must be able to point out the forged ICMP Redirect packet, identify the attacker and victim IPs, and explain how this alters the victim's routing table, often redirecting their traffic through the attacker's machine or blackholing it.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
A common exam exercise involves writing stateful firewall rules based on a given network topology and specific security requirements. You will be asked to complete a table with these rules, ensuring proper stateful inspection (e.g., allowing established connections). Additionally, you may need to propose topology changes (like adding a DMZ, VPN, HTTPS, or NAT) to prevent specific attacks, and evaluate how effective an attack would be after these network changes are applied.
\end{tcolorbox}
