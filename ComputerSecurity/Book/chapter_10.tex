...
Expect questions where you must complete a table with stateful firewall rules based on a given network topology and specific security requirements. You may be provided with a network diagram showing different zones (e.g., DMZ, Internal, External). Pay close attention to whether the firewall is stateful or stateless. For stateful firewalls, you typically only need to specify the rule for the connection initiation; the return traffic for established sessions is automatically permitted by the state table. Remember to rely on the ``default deny'' base policy and carefully track the state of connections for inbound and outbound traffic flows.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
You may be asked to propose network mitigations or topology changes (such as introducing a DMZ, VPN, NAT, SYN-cookies, HTTPS, or specific port security) to prevent specific attacks. Be explicit about the attacks they prevent. For example, mention how a VPN provides confidentiality over an untrusted network, how a DMZ isolates public-facing servers from the internal LAN, or how SYN-cookies mitigate SYN flooding attacks. Be prepared to evaluate the effectiveness of an attack before and after such architectural changes, explaining precisely why the mitigation disrupts the attack chain.
\end{tcolorbox}
