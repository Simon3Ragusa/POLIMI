...
Expect questions asking you to identify evasive techniques directly from assembly or pseudo-code, such as polymorphic/metamorphic behavior, packing, anti-debugging, dormant/event-triggered code, or time-bombs. You may also be asked to identify flaws in badly implemented anti-analysis techniques and explain how a defender could exploit those flaws. Another typical task is modifying assembly code to bypass signature matching (e.g., by manually applying metamorphic transformations).
\end{tcolorbox}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Exam Insight]
A frequent exam scenario requires discussing whether \textbf{Static} or \textbf{Dynamic} analysis is more appropriate for a given malware sample, and detailing the key elements to observe. Furthermore, you should be ready to contrast \textbf{signature-based detection} with \textbf{heuristics}.
\end{tcolorbox}
